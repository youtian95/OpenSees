...
// 用法: 脚本命令
//       uniaxialMaterial MasonryBendingMat matTag? Ke? Mmax? <umax?> <uu?> <R_My?> <CF?> <CD?> <gamma1?> <gamma2?> <postUltimateStiffness?>
//       Ke: 弹性刚度; Mmax: 最大弯矩; umax: 最大弯矩对应转角(默认0.004); uu: 峰值平台结束转角(默认0.008)
//       R_My: 屈服弯矩与Mmax的比值(论文取0.7)
//       CF: 第一段卸载相对反转点弯矩的下降比例(论文取0.2)
//       CD: 卸载第二折点相对屈服转角的水平位置参数(论文取0.1)
//       gamma1: 最大弯矩对应转角处第一段卸载刚度与Ke之比
...
class MasonryBendingMat : public UniaxialMaterial
{
  public:
    // 构造函数
    // tag: 材料标签; Ke Mmax 必选; umax~postUltimateStiffness 可选(默认值见参数声明)
    MasonryBendingMat(int tag, double Ke, double Mmax, double umax = 0.004, double uu = 0.008, double R_My = 0.7, double CF = 0.2, double CD = 0.1, double gamma1 = 1.2, double gamma2 = 1.2, double postUltimateStiffness = 0.0);
  ...
  private:
    ...
    double umax;    // 最大弯矩对应转角
    double uu;      // 峰值平台结束、峰后刚度开始作用的转角
    double R_My;    // 屈服弯矩与Mmax的比值
    double CF;      // 第一段卸载段相对反转点的力下降比
    double CD;      // 第二折点相对屈服转角的水平位置参数
    double gamma1;  // 最大弯矩对应转角处第一段卸载刚度与Ke之比
