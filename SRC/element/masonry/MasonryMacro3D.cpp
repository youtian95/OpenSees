    ...
    while (remainingArguments > 0) {
        ...
        if (std::strcmp(option, "-ForPrintModelJson") == 0) {
            if (remainingArguments < 2) {
                opserr << "WARNING masonryMacro3D -ForPrintModelJson requires width and thickness" << endln;
                return 0;
            }
            int numberOfData = 2;
            if (OPS_GetDoubleInput(&numberOfData, printModelData) < 0) {
                opserr << "WARNING masonryMacro3D failed to read -ForPrintModelJson parameters" << endln;
                return 0;
            }
        ...
        if (remainingArguments < 5) {
            opserr << "WARNING masonryMacro3D -AxialForceInteraction requires width thickness fm cohesion ft <kd> <mu>" << endln;
            return 0;
        }
...
bool MasonryMacro3D::hasTrialLateralFailure(void) const
{
